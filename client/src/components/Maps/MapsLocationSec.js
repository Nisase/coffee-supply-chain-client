...
function SendLocation({ marker, location }) {
  ...
  const sendLoc = () => {
    dispatch(setDirectionDataSec(location));
    dispatch(setLatitudeDataSec(marker.lat));
    dispatch(setLongitudeDataSec(marker.lng));
    dispatch(setLocReadyToAddDataSec(true));
    enqueueSnackbar(
...
export default MapsLocationSec;
